...
def logout():
    ...
    if session.get('user'): # Used MS Login
        ...
        redirect(Config.AUTHORITY + '/oauth2/v2.0/logout' +
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        result=msal_app.acquire_token_by_authorization_code(code=request.args.get('code'), scopes=Config.SCOPE)
        print(result)
...
def _build_msal_app(cache=None, authority=None):
    ...
    msal_app = msal.ConfidentialClientApplication(client_id=Config.CLIENT_ID, client_credential=Config.CLIENT_SECRET, authority=authority)
    return msal_app
...
def _build_auth_url(authority=None, scopes=None, state=None):
    ...
    return msal_app.get_authorization_request_url(scopes=scopes, state=state)
